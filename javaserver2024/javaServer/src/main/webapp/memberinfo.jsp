...
<body>
	...
	<table border="1">
		<tr>
			<th>번호</th><th>이름</th><th>회사</th><th>이메일</th><th>가입날짜</th>
		</tr>
		<c:forEach var="m" varStatus="i" items="${memberlist}">
			<tr>
				<td>${m.id}</td><td>${m.username}</td>
				<td>${m.company}</td><td>${m.email}</td><td>${birthday}</td>
			</tr>
		</c:forEach>
	</table>
	...
	<h2>회원 추가</h2>
	<hr>
	<form method="post" action="/javaServer/mcontroller?action=insert">
